...
function validateCred(cardNo){
  ...
  let nSum = 0;
  for (let i = nDigits - 1, ctr = 0; i >= 0; i--)
  {
    ctr++;
    let d = cardNo[i];
    if(ctr % 2 === 0){
...
function findInvalidCards(cardNumbers){
  const invalidNumbers = cardNumbers.filter((number)=>{
...
/**
 * check invalid companies from card numbers provided
 * return invalid companies
*/
function idInvalidCardCompanies(cardNumbers){
  let companies = [];
  //check if digit is equivalent to the company
  cardNumbers.forEach((number)=>{
    const fdigit = number[0];
    switch(fdigit){
      case 3:
